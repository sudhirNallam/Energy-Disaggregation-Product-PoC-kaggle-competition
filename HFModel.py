...
def model(freqData, target):
    ...
    f.write("\nkNN model with "+str(neighbors)+" neighbors.")
    ...
    f.write("\nAccuracy on test = %.7f" % metrics.accuracy_score(prediction, Y_test))
    f.write("\nHamming loss on test = %.7f" % metrics.hamming_loss(prediction, Y_test))
